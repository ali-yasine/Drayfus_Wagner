...
int main() {
    ...
    //     std ::cout << std::endl;
    // }
    unsigned int rowPtr[] {0,2,3,4,4};
    unsigned int col[] {1, 3, 2, 3};
    unsigned int values[] {5, 10, 3, 1};
    ...
    printf("starting\n");
    unsigned int* result = floydWarshall(graph);
    for(unsigned int vertex = 0; vertex < 4; vertex++) {
        for(unsigned int subset = 0; subset < 4; subset++) {
            std::cout << result[vertex * 4 + subset] << " ";
        }
        std ::cout << std::endl;
    }
}
