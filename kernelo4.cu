...
__global__ void DW_kernel_o4(CsrGraph* graph, unsigned int numTerminals, unsigned int* terminals, unsigned int* DP, unsigned int* apsp, unsigned int* allSubsets, unsigned int numSubsets, unsigned int coarseFactor, unsigned int k, unsigned int subsetsDoneSoFar) {
...
void DrayfusWagnerGPU_o4(CsrGraph* graph_cpu, CsrGraph* graph, unsigned int numTerminals, unsigned int* terminals, unsigned int* DP, unsigned int* apsp) {
    ...
    for(unsigned int k = 2; k <= numTerminals; ++k) {
        ...
        if (sharedMemPerBlock > MAX_SHARED_MEM) {
            ...
            DW_kernel<<<numBlocks, numThreads>>>(graph, numTerminals, terminals_d, DP_d, apsp_d, allSubsets_d, numSubsets, coarseFactor, k, subsetsDoneSoFar, subSubSets_s);
            
            cudaFree(subSubSets_s);
        }

        else 
            DW_kernel_o4<<<numBlocks, numThreads, sharedMemPerBlock>>>(graph, numTerminals, terminals_d, DP_d, apsp_d, allSubsets_d, numSubsets, coarseFactor, k, subsetsDoneSoFar);
    ...
    stopTime(&timer);
    printElapsedTime(timer, "Kernel opt4 time: ", GREEN);
